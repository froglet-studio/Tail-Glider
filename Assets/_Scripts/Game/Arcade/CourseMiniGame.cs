...
namespace CosmicShore.Game.Arcade
{
	public class CourseMiniGame : MiniGame
	{
		...
		{
			base.Start();
			if (ScaleNumberOfSegmentsWithIntensity)
			{
				numberOfSegments *= IntensityLevel;
			}

			// TODO: remove special cases for specific ships.
			// This belongs in the ship's class, not here.
			...
			if (helix)
			{
				helix.firstOrderRadius = helix.secondOrderRadius = IntensityLevel / 1.3f;
			}
		...
		void InitializeTrails()
		{
			...
			if (ScaleLengthWithIntensity)
			{
				SegmentSpawner.StraightLineLength = straightLineLength / IntensityLevel;
			}

			TrailSpawner.NukeTheTrails();
			if (ScaleCrystalPositionWithIntensity)
			{
				Crystal.transform.position = IntensityLevel * CrystalStartPosition;
			}
			else
			{
